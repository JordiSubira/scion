...
type Level1PrefetchListKeeper interface {
	...
	GetLevel1InfoArray() []Level1PrefetchInfo
}

// Level1PrefetchInfo contains the information to prefetch lvl1 keys from remote CSes.
...
func (s *serviceEngine) GetLevel1PrefetchInfo() []Level1PrefetchInfo {
	return s.prefetchKeeper.GetLevel1InfoArray()
}
...
func (s *serviceEngine) obtainLevel1Key(
	...
	return s.getLevel1Key(ctx, lvl1Meta)
...
func deriveLevel1(meta drkey.Level1Meta, sv drkey.SecretValue) (drkey.Level1Key, error) {
	...
	if err != nil {
		return drkey.Level1Key{}, serrors.WrapStr("computing lvl1 raw key", err)
	}
	return drkey.Level1Key{
		Epoch:   sv.Epoch,
		SrcIA:   meta.SrcIA,
...
func (s *serviceEngine) DeriveASHost(
	...
	lvl1Key, err := s.obtainLevel1Key(ctx, meta.ProtoId, meta.Validity, meta.SrcIA, meta.DstIA)
	if err != nil {
		return drkey.ASHostKey{}, serrors.WrapStr("getting  lvl1 key", err)
	}

	if meta.ProtoId.IsPredefined() {
		key, err = (&specific.Deriver{}).DeriveASHost(meta.DstHost, lvl1Key.Key)
		if err != nil {
...
func (s *serviceEngine) DeriveHostAS(
	...
	if meta.ProtoId.IsPredefined() {
		key, err = (&specific.Deriver{}).DeriveHostAS(meta.SrcHost, lvl1Key.Key)
		if err != nil {
			return drkey.HostASKey{}, serrors.WrapStr("parsing derivation input", err)
		}
	} else {
		key, err = (&generic.Deriver{}).DeriveHostAS(meta.ProtoId, meta.SrcHost, lvl1Key.Key)
		if err != nil {
			return drkey.HostASKey{}, serrors.WrapStr("parsing derivation input", err)
		}
	}
...
func (s *serviceEngine) DeriveHostHost(
	...
	if meta.ProtoId.IsPredefined() {
		key, err = (&specific.Deriver{}).DeriveHostHost(meta.DstHost, hostASKey.Key)
		if err != nil {
			return drkey.HostHostKey{}, serrors.WrapStr("parsing derivation input", err)
		}
	} else {
		key, err = (&generic.Deriver{}).DeriveHostHost(meta.DstHost, hostASKey.Key)
		if err != nil {
			return drkey.HostHostKey{}, serrors.WrapStr("parsing derivation input", err)
		}
	}
